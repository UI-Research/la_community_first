...
```{r}
boulevard = "samo"
boulevard_tracts = file.path(file_path, boulevard, "maps", str_c(boulevard, "_tracts.shp")) %>%
  st_read(quiet = TRUE) #%>% 
  #st_set_crs(4326)
```
...
acs_tracts_boulevard = acs_tracts_la %>%
  ...
  filter(str_starts(GEOID, c("0603726|0603798")))
...
color_palette = c(
  "LA City" = "#00447c",
  "Santa Monica to Westwood" = "#A7c539")
...
comparison_charts = plot_comparison_charts(
  ...
  df_city = acs_city_la,
  boulevard_name = "Santa Monica to Westwood",
  color = color_palette,
...
streets_sf = tribble(
  ...
  #### SaMo Locations ####
  "Wilshire Blvd", 34.03599619998233, -118.47751443335834, 46, boulevard,
  ...
  "San Vicente Blvd", 34.05190061515859, -118.47460501575662, 18, boulevard,
  "Santa Monica Blvd", 34.033194769667645, -118.4743223448754, 46, boulevard,
  ...
  "Bundy Dr", 34.03743794455129, -118.45977952761376, 320, boulevard
...
locations_sf = tribble(
  ...
  #### SaMo Locations ####
  "UCLA", 34.06993161179595, -118.44678711417353, boulevard,
...
boulevard_outline <- st_union(boulevard_tracts) #%>%
  #st_transform(projection)
boulevard_corridor <- st_read(file.path(file_path, boulevard, "maps", paste0(boulevard, "_corridor.shp"))) %>%
  st_transform(4326) 
```
...
disability_counts <- acs_tracts_boulevard |>
  ...
  mutate(percent = Count/ sum(6015) * 100,
...
hla_combined <- bind_rows(hla_boulevard, hla_all_city) %>%
  ...
  mutate(variable = "Share of affirmative votes",
         area = factor(area, levels = c("Santa Monica to Westwood", "LA City")))


hla_bar <- ggplot(hla_combined) +
  geom_col(aes(x = variable, y = value, fill = area), position = position_dodge(width = 0.8)) + 
  scale_fill_manual(values = c("LA City" = "#00447C", "Santa Monica to Westwood" = "#A7C539")) +
...
hla_supplement <- read_csv(file.path(file_path, "Data/Measure HLA Vote/hla_participation_data.csv")) %>% 
  filter(TYPE == "TOTAL") %>% 
...
hla_supplement_boulevard <- read_csv(file.path(file_path, "Data/Measure HLA Vote/hla_participation_data.csv")) %>% 
