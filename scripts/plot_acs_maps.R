...
plot_acs_maps = function(
  ...
  sf = sf %>%
    ...
    mutate(
      ...
      share_no_int = B28002_013/B28002_001,
      bin_no_int = 
        cut(
          share_no_int, 
          ...
          include.lowest = TRUE) %>%
        factor(levels = c("Less than 5%", "5-10%", "10-15%", "15-20%", "20% or more")),
  ...
  plot_metadata = data.frame(
    fill_column = c(
      ...
      "bin_disability",  ## disability
      "bin_hearing_diff", ## hearing diff
      "bin_vision_diff", ## vision diff
      "bin_ambulatory_diff", ## ambulatory diff
      "B19013_001", ## median_income
      "pov_bin", ## poverty
      "bin_no_int", ## no internet
      "bin_no_computer", ## no computer
      "bin_tenure", ## housing tenure
      "bin_no_car", ## no car
      "bin_hour_plus",## commute time
      "C16001_018", ## korean language
      "C16001_003", # spanish language 
      "bin_renter_housing_costburden_30plus", ## renter housing cost burden 30%
      "bin_renter_housing_costburden_50plus", ## renter housing cost burden 50%
  ...
  constructs = c("population", "disability", "disability_hearing", "disability_vision", 
                 "disability_ambulatory", "median_income", "poverty", "no_internet", 
