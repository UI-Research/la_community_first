###load list of 2023 acs variables
v23 <- load_variables(2023, "acs5", cache = TRUE)

### Load the script for pico data ###
source("C:/Users/tmaginn/Documents/GitHub/la_community_first/data_pull.R")
source("C:/Users/gsamuels/Documents/GitHub/la_community_first/data_pull.R")



# Set working directory and file paths
username <- getwd() %>% str_split("\\/") %>% unlist() %>% .[3]
file_path <- file.path("C:", "Users", username, "Box", "LA Transit project/Social Climate Analysis")


# Call the function and store the result
pico_master_data <- clean_and_prepare_data(file_path)


### same thing as above but now loading in script for LA city data ###

source("C:/Users/tmaginn/Documents/GitHub/la_community_first/la_city_data_pull.R")
source("C:/Users/gsamuels/Documents/GitHub/la_community_first/la_city_data_pull.R")


username <- getwd() %>% str_split("\\/") %>% unlist() %>% .[3]
file_path <- file.path("C:", "Users", username, "Box", "LA Transit project/Social Climate Analysis")

# Call the function and store the result
la_city_master_data <- clean_and_prepare_data(file_path)




################ Making charts ##############

###load list of 2023 acs variables
v23 <- load_variables(2023, "acs5", cache = TRUE)

######################################################## age distribution #######################################################################

#define age bins
age_bins <- list(
  age_0_17  = c("B01001_003", "B01001_004", "B01001_005", "B01001_006",
                "B01001_027", "B01001_028", "B01001_029", "B01001_030"),
  age_18_29 = c("B01001_007", "B01001_008", "B01001_009", "B01001_010", "B01001_011",  
                "B01001_031", "B01001_032", "B01001_033", "B01001_034", "B01001_035"),
  age_30_44 = c("B01001_012", "B01001_013", "B01001_014", 
                "B01001_036", "B01001_037", "B01001_038"),
  age_45_59 = c("B01001_015", "B01001_016", "B01001_017", 
                "B01001_039", "B01001_040", "B01001_041"),
  age_60_up = c("B01001_018", "B01001_019", "B01001_020", "B01001_021", "B01001_022", "B01001_023", 
                "B01001_024", "B01001_025", "B01001_042", "B01001_043", "B01001_044", "B01001_045", 
                "B01001_046", "B01001_047", "B01001_048", "B01001_049")
)

#sum total population and sum population for each age bin, then compute shares
age_pico <- pico_master_data %>%
  summarize(
    total_population = sum(B01001_001, na.rm = TRUE),
    
    age_0_17  = sum(across(all_of(age_bins$age_0_17)),  na.rm = TRUE),
    age_18_29 = sum(across(all_of(age_bins$age_18_29)), na.rm = TRUE),
    age_30_44 = sum(across(all_of(age_bins$age_30_44)), na.rm = TRUE),
    age_45_59 = sum(across(all_of(age_bins$age_45_59)), na.rm = TRUE),
    age_60_up = sum(across(all_of(age_bins$age_60_up)), na.rm = TRUE)
  ) %>%
  mutate(
    share_age_0_17  = age_0_17  / total_population,
    share_age_18_29 = age_18_29 / total_population,
    share_age_30_44 = age_30_44 / total_population,
    share_age_45_59 = age_45_59 / total_population,
    share_age_60_up = age_60_up / total_population
  )

#LA city age shares
age_la <- la_city_master_data %>%
  summarize(
    total_population = sum(B01001_001, na.rm = TRUE),
    
    age_0_17  = sum(across(all_of(age_bins$age_0_17)),  na.rm = TRUE),
    age_18_29 = sum(across(all_of(age_bins$age_18_29)), na.rm = TRUE),
    age_30_44 = sum(across(all_of(age_bins$age_30_44)), na.rm = TRUE),
    age_45_59 = sum(across(all_of(age_bins$age_45_59)), na.rm = TRUE),
    age_60_up = sum(across(all_of(age_bins$age_60_up)), na.rm = TRUE)
  ) %>%
  mutate(
    share_age_0_17  = age_0_17  / total_population,
    share_age_18_29 = age_18_29 / total_population,
    share_age_30_44 = age_30_44 / total_population,
    share_age_45_59 = age_45_59 / total_population,
    share_age_60_up = age_60_up / total_population
  )

## making the bar chart

#combine and reshape both dataframes

# Add area labels
pico_long <- age_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "age_group", values_to = "share")

city_long <- age_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "age_group", values_to = "share")

# Combine
combined <- bind_rows(pico_long, city_long)

#custom labels for age groups and city
combined <- combined %>%
  mutate(
    age_group = recode(age_group,
                       "share_age_0_17"  = "0–17",
                       "share_age_18_29" = "18–29",
                       "share_age_30_44" = "30–44",
                       "share_age_45_59" = "45–59",
                       "share_age_60_up" = "60+"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )

# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))

#plot
ggplot(combined, aes(x = age_group, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          # inside the bar, near top but inside
            color = "white",      # white text
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )



################################################################# ethnicity ############################################################

ethnicity_vars <- list(
  total = "B03002_001",
  white = "B03002_003",
  black = "B03002_004",
  indigenous = "B03002_005",
  asian = "B03002_006",
  pacific = "B03002_007",
  other = "B03002_010",
  hispanic = "B03002_012"
)


# Pico ethnicity shares
ethnicity_pico <- pico_master_data %>%
  summarize(
    total_population = sum(.data[[ethnicity_vars$total]], na.rm = TRUE),
    white       = sum(.data[[ethnicity_vars$white]], na.rm = TRUE),
    black       = sum(.data[[ethnicity_vars$black]], na.rm = TRUE),
    indigenous  = sum(.data[[ethnicity_vars$indigenous]], na.rm = TRUE),
    asian       = sum(.data[[ethnicity_vars$asian]], na.rm = TRUE),
    pacific     = sum(.data[[ethnicity_vars$pacific]], na.rm = TRUE),
    other       = sum(.data[[ethnicity_vars$other]], na.rm = TRUE),
    hispanic    = sum(.data[[ethnicity_vars$hispanic]], na.rm = TRUE)
  ) %>%
  mutate(
    across(white:hispanic, ~ .x / total_population, .names = "share_{.col}")
  )

# LA City ethnicity shares
ethnicity_la <- la_city_master_data %>%
  summarize(
    total_population = sum(.data[[ethnicity_vars$total]], na.rm = TRUE),
    white       = sum(.data[[ethnicity_vars$white]], na.rm = TRUE),
    black       = sum(.data[[ethnicity_vars$black]], na.rm = TRUE),
    indigenous  = sum(.data[[ethnicity_vars$indigenous]], na.rm = TRUE),
    asian       = sum(.data[[ethnicity_vars$asian]], na.rm = TRUE),
    pacific     = sum(.data[[ethnicity_vars$pacific]], na.rm = TRUE),
    other       = sum(.data[[ethnicity_vars$other]], na.rm = TRUE),
    hispanic    = sum(.data[[ethnicity_vars$hispanic]], na.rm = TRUE)
  ) %>%
  mutate(
    across(white:hispanic, ~ .x / total_population, .names = "share_{.col}")
  )

# Reshape to long format
pico_long <- ethnicity_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "group", values_to = "share")

city_long <- ethnicity_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "group", values_to = "share")

# Combine
ethnicity_combined <- bind_rows(pico_long, city_long) %>%
  mutate(
    group = recode(group,
                   "share_white"      = "White",
                   "share_black"      = "Black",
                   "share_indigenous" = "Indigenous",
                   "share_asian"      = "Asian",
                   "share_pacific"    = "Pacific Islander",
                   "share_other"      = "Other",
                   "share_hispanic"   = "Hispanic"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )

colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))


# Reorder ethnicity groups by share
ethnicity_order <- ethnicity_combined %>%
  filter(area == "LA City") %>%
  arrange(desc(share)) %>%
  pull(group)

ethnicity_combined <- ethnicity_combined %>%
  mutate(group = factor(group, levels = ethnicity_order))

ggplot(ethnicity_combined, aes(x = group, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,
            color = "white",
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )


########################################### employment status ###########################################
employment_bins <- list(
  labor_force = c("B23001_006", "B23001_013", "B23001_020", "B23001_027", "B23001_034", "B23001_041", "B23001_048", "B23001_055", "B23001_062", "B23001_069",
                  "B23001_074", "B23001_079", "B23001_084", "B23001_092", "B23001_099", "B23001_106", "B23001_113", "B23001_120", "B23001_127", "B23001_134",
                  "B23001_141", "B23001_148", "B23001_155", "B23001_160", "B23001_165", "B23001_170"),
  employed = c("B23001_007", "B23001_014", "B23001_021", "B23001_028", "B23001_035", "B23001_042", "B23001_049", "B23001_056", "B23001_063", "B23001_070",
               "B23001_075", "B23001_080", "B23001_085", "B23001_093", "B23001_100", "B23001_107", "B23001_114", "B23001_121", "B23001_128", "B23001_135",
               "B23001_142", "B23001_149", "B23001_156", "B23001_161", "B23001_166", "B23001_171"),
  unemployed =  c("B23001_008", "B23001_015", "B23001_022", "B23001_029", "B23001_036", "B23001_043", "B23001_050", "B23001_057", "B23001_064", "B23001_071",
                  "B23001_076", "B23001_081", "B23001_086", "B23001_094", "B23001_101", "B23001_108", "B23001_115", "B23001_122", "B23001_129", "B23001_136",
                  "B23001_143", "B23001_150", "B23001_157", "B23001_162", "B23001_167", "B23001_172"),
  not_in_labor_force =  c("B23001_009", "B23001_016", "B23001_023", "B23001_030", "B23001_037", "B23001_044", "B23001_051", "B23001_058", "B23001_065", "B23001_072",
                  "B23001_077", "B23001_082", "B23001_087", "B23001_095", "B23001_102", "B23001_109", "B23001_116", "B23001_123", "B23001_130", "B23001_137",
                  "B23001_144", "B23001_151", "B23001_158", "B23001_163", "B23001_168", "B23001_173")
)


#pico employment shares
employment_pico <- pico_master_data %>%
  summarize(
    total = sum(B23001_001, na.rm = T),
    
    labor_force = sum(across(all_of(employment_bins$labor_force)), na.rm = T),
    employed = sum(across(all_of(employment_bins$employed)), na.rm = T),
    unemployed = sum(across(all_of(employment_bins$unemployed)), na.rm = T),
    not_in_labor_force = sum(across(all_of(employment_bins$not_in_labor_force)), na.rm = T)
  )%>%
  mutate(
    share_employed = employed/labor_force,
    share_unemployed = unemployed/total,
    share_no_lf = not_in_labor_force/total
  )

#la employment shares
employment_la <- la_city_master_data %>%
  summarize(
    total = sum(B23001_001, na.rm = T),
    
    labor_force = sum(across(all_of(employment_bins$labor_force)), na.rm = T),
    employed = sum(across(all_of(employment_bins$employed)), na.rm = T),
    unemployed = sum(across(all_of(employment_bins$unemployed)), na.rm = T),
    not_in_labor_force = sum(across(all_of(employment_bins$not_in_labor_force)), na.rm = T)
  )%>%
  mutate(
    share_employed = employed/labor_force,
    share_unemployed = unemployed/total,
    share_no_lf = not_in_labor_force/total
  )  


# Reshape and combine dataframes
pico_long <- employment_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "employment", values_to = "share")

city_long <- employment_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "employment", values_to = "share")


combined <- bind_rows(pico_long, city_long)

#custom labels
combined <- combined %>%
  mutate(
    employment = recode(employment,
                        "share_employed" = "Employed",
                        "share_unemployed" = "Unemployed",
                        "share_no_lf" = "Not in labor force"
    ),
    # Convert to factor with custom order
    employment = factor(employment, levels = c(
      "Employed",
      "Unemployed",
      "Not in labor force"
    )),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )


#LA DOT colors
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))

#plot
ggplot(combined, aes(x = employment, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          # inside the bar, near top but inside
            color = "white",      # white text
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )



###################################### Gender ######################################
#obtain gender counts for pico and LA City
pico_gender <- pico_master_data %>%
  summarize(
    total = sum(B01001_001, na.rm = T),
    male = sum(B01001_002, na.rm = T),
    female = sum(B01001_026, na.rm = T)
  )%>%
      mutate(
        across(male:female, ~ .x/ total, .names = "share_{.col}")
  )

la_gender <- la_city_master_data %>%
  summarize(
    total = sum(B01001_001, na.rm = T),
    male = sum(B01001_002, na.rm = T),
    female = sum(B01001_026, na.rm = T)
  )%>%
  mutate(
    across(male:female, ~ .x/ total, .names = "share_{.col}")
  )


# Reshape and combine dataframes
pico_long <- pico_gender %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "group", values_to = "share")

city_long <- la_gender %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "group", values_to = "share")

gender_combined <- bind_rows(pico_long, city_long) %>%
  mutate(
    group = recode(group,
                   "share_male" = "Male",
                   "share_female" = "Female"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(scales::number(share * 100, accuracy = 0.1), "%")
    
  )

colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))


#plotting
ggplot(gender_combined, aes(x = group, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,
            color = "white",
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )




####################################################### Family Structure ##############################################

#defining bins
family_bins <- list(
  married_children = "B11003_003",
  married_no_children = "B11003_007",
  single_children = c("B11003_010", "B11003_016"),
  single_no_children = c("B11003_014", "B11003_020")
)


#pico and la city shares
pico_family <- pico_master_data%>%
  summarize(
    total = sum(B11003_001,na.rm= T),
    
    married_children = sum(across(all_of(family_bins$married_children)), na.rm = T),
    married_no_children = sum(across(all_of(family_bins$married_no_children)), na.rm = T),
    single_children = sum(across(all_of(family_bins$single_children)), na.rm = T),
    single_no_children = sum(across(all_of(family_bins$single_no_children)), na.rm = T)
  ) %>%
  mutate(
    share_married_children = married_children/total,
    share_married_no_children = married_no_children/total,
    share_single_children = single_children/total,
    share_single_no_children = single_no_children/total
  )

la_family <- la_city_master_data%>%
  summarize(
    total = sum(B11003_001,na.rm= T),
    
    married_children = sum(across(all_of(family_bins$married_children)), na.rm = T),
    married_no_children = sum(across(all_of(family_bins$married_no_children)), na.rm = T),
    single_children = sum(across(all_of(family_bins$single_children)), na.rm = T),
    single_no_children = sum(across(all_of(family_bins$single_no_children)), na.rm = T)
  ) %>%
  mutate(
    share_married_children = married_children/total,
    share_married_no_children = married_no_children/total,
    share_single_children = single_children/total,
    share_single_no_children = single_no_children/total
  )

#transform and combine data
pico_long <- pico_family %>%
  select(starts_with("share_"))%>%
         mutate(area = "Pico")%>%
           pivot_longer(cols = starts_with("share_"),
                        names_to = "fam_structure", values_to = "share")

city_long <- la_family %>%
  select(starts_with("share_"))%>%
  mutate(area = "LA City")%>%
  pivot_longer(cols = starts_with("share_"),
               names_to = "fam_structure", values_to = "share")          

combined <- bind_rows(pico_long, city_long)

#custom labels
combined<- combined %>%
  mutate(
    fam_structure = recode(fam_structure,
                       "share_married_children" = "Married with children",
                       "share_married_no_children" = "Married without children",
                       "share_single_children" = "Single parent with children",
                       "share_single_no_children" = "Single without children"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )


#LA DOT colors
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))

#plot
ggplot(combined, aes(x = fam_structure, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          # inside the bar, near top but inside
            color = "white",      # white text
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )



############################################### English speaking ability ###############################################
language_bins <- list(
  very_well = c("B16004_005", "B16004_010", "B16004_015", "B16004_020", "B16004_027", "B16004_032", "B16004_037", "B16004_042", "B16004_049", "B16004_054", "B16004_059", "B16004_064"),
  well = c("B16004_006", "B16004_011", "B16004_016", "B16004_021", "B16004_028", "B16004_033", "B16004_038", "B16004_043", "B16004_050", "B16004_055", "B16004_060", "B16004_065"),
  not_well = c("B16004_007", "B16004_012", "B16004_017", "B16004_022", "B16004_029", "B16004_034", "B16004_039", "B16004_044", "B16004_051", "B16004_056", "B16004_061", "B16004_066"),
  none =c("B16004_008", "B16004_013", "B16004_018", "B16004_023", "B16004_030", "B16004_035", "B16004_040", "B16004_045", "B16004_052", "B16004_057", "B16004_062", "B16004_067")
)

#data for pico and la city
english_pico <- pico_master_data %>%
  summarize(
    total = sum(B16004_001, na.rm = T),
    
    very_well = sum(across(all_of(language_bins$very_well)), na.rm = T),
    well = sum(across(all_of(language_bins$well)), na.rm = T),
    not_well = sum(across(all_of(language_bins$not_well)), na.rm = T),
    none = sum(across(all_of(language_bins$none)), na.rm = T)
  ) %>%
  mutate(
    share_very_well = very_well/total,
    share_well = well/total,
    share_not_well = not_well/total,
    share_none = none/total
  ) 


english_la <- la_city_master_data %>%
  summarize(
    total = sum(B16004_001, na.rm = T),
    
    very_well = sum(across(all_of(language_bins$very_well)), na.rm = T),
    well = sum(across(all_of(language_bins$well)), na.rm = T),
    not_well = sum(across(all_of(language_bins$not_well)), na.rm = T),
    none = sum(across(all_of(language_bins$none)), na.rm = T)
  ) %>%
  mutate(
    share_very_well = very_well/total,
    share_well = well/total,
    share_not_well = not_well/total,
    share_none = none/total
  ) 

#reshape and combine df's
pico_long <- english_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "ability", values_to = "share")

city_long <- english_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "ability", values_to = "share")

combined <- bind_rows(pico_long, city_long)

#custom labels for age groups and city
combined <- combined %>%
  mutate(
    ability = recode(ability,
                       "share_very_well"  = "Very well",
                       "share_well" = "Well",
                       "share_not_well" = "Not well",
                       "share_none" = "None"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )%>%
  mutate(ability = factor(ability, levels = c(
    "Very well",
    "Well",
    "Not well",
    "None"
  )))

# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))

#plot
ggplot(combined, aes(x = ability, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          
            color = "white",    
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )




##########################################Primary language (we specifically care about the languages spoken for people who do not speak english well)###########
language_bins <- list(
  english_only = "C16001_002",
  spanish= "C16001_005",
  korean = "C16001_020",
  chinese = "C16001_023",
  other = c("C16001_008", "C16001_011", "C16001_014", "C16001_017", "C16001_026", "C16001_029", "C16001_032", "C16001_035", "C16001_038")
)

#gather data
language_pico <- pico_master_data %>%
  summarize(
    total = sum(C16001_001, na.rm = T),
    
    english_only = sum(across(all_of(language_bins$english_only)), na.rm = T),
    spanish = sum(across(all_of(language_bins$spanish)), na.rm = T),
    korean = sum(across(all_of(language_bins$korean)), na.rm = T),
    chinese = sum(across(all_of(language_bins$chinese)), na.rm = T),
    other = sum(across(all_of(language_bins$other)), na.rm = T)
  ) %>%
  mutate(
    share_english_only = english_only/total,
    share_spanish = spanish/total,
    share_korean = korean/total,
    share_chinese = chinese/total,
    share_other = other/total
  ) 

language_la <- la_city_master_data %>%
  summarize(
    total = sum(C16001_001, na.rm = T),
    
    english_only = sum(across(all_of(language_bins$english_only)), na.rm = T),
    spanish = sum(across(all_of(language_bins$spanish)), na.rm = T),
    korean = sum(across(all_of(language_bins$korean)), na.rm = T),
    chinese = sum(across(all_of(language_bins$chinese)), na.rm = T),
    other = sum(across(all_of(language_bins$other)), na.rm = T)
  ) %>%
  mutate(
    share_english_only = english_only/total,
    share_spanish = spanish/total,
    share_korean = korean/total,
    share_chinese = chinese/total,
    share_other = other/total
  ) 


#reshape and combine
pico_long <- language_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "language", values_to = "share")

city_long <- language_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "language", values_to = "share")

combined <- bind_rows(pico_long, city_long)


#custom labels
combined <- combined %>%
  mutate(
    language = recode(language,
                     "share_english_only"  = "English only",
                     "share_spanish" = "Spanish",
                     "share_korean" = "Korean",
                     "share_chinese" = "Chinese",
                     "share_other" = "Other languages"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  ) %>%
  mutate(language = factor(language, levels = c(
    "English only",
    "Spanish",
    "Korean",
    "Chinese",
    "Other languages"
  )))

# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))

#plot
ggplot(combined, aes(x = language, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = ifelse(share >= 0.03, label, "")), #removing the percent label for small values (< 3%) because it gets cut off
            position = position_dodge(width = 0.8),
            vjust = 1.5,          
            color = "white",    
            size = 4)+
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )





########################################################### device/internet access #########################################################

device_bins <- list(
  no_smartphone = c("B28001_004", "B28001_008", "B28001_010"),
  no_computer = c("B28001_006", "B28001_008", "B28001_010"), 
  no_internet = "B28002_013"
)

#gather data
device_pico <- pico_master_data %>%
  summarize(
    total = sum(B28001_001, na.rm = T),
    
    no_smartphone = sum(across(all_of(device_bins$no_smartphone)), na.rm = T),
    no_computer = sum(across(all_of(device_bins$no_computer)), na.rm = T),
    no_internet = sum(across(all_of(device_bins$no_internet)), na.rm = T)
    
  ) %>%
  mutate(
    share_no_smartphone = no_smartphone/total,
    share_no_computer = no_computer/total,
    share_no_internet = no_internet/total
  )
  

device_la <- la_city_master_data %>%
  summarize(
    total = sum(B28001_001, na.rm = T),
    
    no_smartphone = sum(across(all_of(device_bins$no_smartphone)), na.rm = T),
    no_computer = sum(across(all_of(device_bins$no_computer)), na.rm = T),
    no_internet = sum(across(all_of(device_bins$no_internet)), na.rm = T)
    
  ) %>%
  mutate(
    share_no_smartphone = no_smartphone/total,
    share_no_computer = no_computer/total,
    share_no_internet = no_internet/total
  )


#reshape and combine df's
pico_long <- device_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "access", values_to = "share")

city_long <- device_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "access", values_to = "share")

combined <- bind_rows(pico_long, city_long)

#custom labels
combined <- combined %>%
  mutate(
    access = recode(access,
                     "share_no_smartphone"  = "No smartphones",
                     "share_no_computer" = "No computers",
                     "share_no_internet" = "No internet access"
    ),
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )%>%
  mutate(access = factor(access, levels = c(
    "No internet access",
    "No smartphones",
    "No computers"
  )))

# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))

#plot
ggplot(combined, aes(x = access, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          
            color = "white",    
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )




############################################################## Commuting modes ##################################################

public_transportation = c("B08301_010", "B08301_011","B08301_012", "B08301_013", "B08301_014", "B08301_015")


#gather data
commute_pico <- pico_master_data %>%
  summarize(
    total = sum(B08301_001, na.rm = T),
    public_transportation = sum(across(all_of(public_transportation)), na.rm = T),
    car =  sum(B08301_002, na.rm = T),
    carpool =  sum(B08301_004, na.rm = T),
    walk =  sum(B08301_019, na.rm = T),
    bike =  sum(B08301_018, na.rm = T),
    wfh =  sum(B08301_021, na.rm = T)
  ) %>%
  mutate(
    share_car = car/total,
    share_pt = public_transportation/total,
    share_carpool = carpool/total,
    share_walk = walk/total,
    share_bike = bike/total,
    share_wfh = wfh/total
  )

commute_la <- la_city_master_data %>%
  summarize(
    total = sum(B08301_001, na.rm = T),
    public_transportation = sum(across(all_of(public_transportation)), na.rm = T),
    car =  sum(B08301_002, na.rm = T),
    carpool =  sum(B08301_004, na.rm = T),
    walk =  sum(B08301_019, na.rm = T),
    bike =  sum(B08301_018, na.rm = T),
    wfh =  sum(B08301_021, na.rm = T)
  ) %>%
  mutate(
    share_car = car/total,
    share_pt = public_transportation/total,
    share_carpool = carpool/total,
    share_walk = walk/total,
    share_bike = bike/total,
    share_wfh = wfh/total
  )


#reshape and combine df's
pico_long <- commute_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "commute", values_to = "share")

city_long <- commute_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "commute", values_to = "share")

combined <- bind_rows(pico_long, city_long)


#custom labels
combined <- combined %>%
  mutate(
    commute = recode(commute,
                     "share_car"  = "Car, truck, or van",
                     "share_pt" = "Public transportation (excluding taxicab)",
                     "share_carpool" = "Carpool",
                     "share_wfh" = "Work from home",
                     "share_walk" = "Walk",
                     "share_bike" = "Bicycle"
    ),
    commute = str_wrap(commute, width = 15),  # wrap first
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  ) %>%
  mutate(
    commute = factor(commute, levels = str_wrap(c(
      "Car, truck, or van",
      "Work from home",
      "Public transportation (excluding taxicab)",
      "Carpool",
      "Walk",
      "Bicycle"
    ), width = 15))  # match the wrapped order
  )


# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))




#plot
ggplot(combined, aes(x = commute, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          
            color = "white",    
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )




################################################################## Commute time ################################################################

commute_bins <- list(
  commute_5_14 = c("B08303_003", "B08303_004"),
  commute_15_24 = c("B08303_005", "B08303_006"),
  commute_25_34 = c("B08303_007", "B08303_008"),
  commute_35_44 = c("B08303_009", "B08303_010")
)

commute_pico <- pico_master_data %>%
  summarize(
    total = sum(B08303_001, na.rm = T),
    less_5 = sum(B08303_002, na.rm = T),
    commute_45_59 = sum(B08303_011, na.rm = T),
    commute_60_89 = sum(B08303_012, na.rm = T),
    commute_90_plus = sum(B08303_013),
    commute_5_14 = sum(across(all_of(commute_bins$commute_5_14)), na.rm = T),
    commute_15_24 = sum(across(all_of(commute_bins$commute_15_24)), na.rm = T),
    commute_25_34 = sum(across(all_of(commute_bins$commute_25_34)), na.rm = T),
    commute_35_44 = sum(across(all_of(commute_bins$commute_35_44)), na.rm = T)
  
  ) %>%
  mutate(
    share_less_5 = less_5/total,
    share_5_14 = commute_5_14/total,
    share_15_24 = commute_15_24/total,
    share_25_34 = commute_25_34/total,
    share_35_44 = commute_35_44/total,
    share_49_59 = commute_45_59/total,
    share_60_89 = commute_60_89/total,
    share_90_plus = commute_90_plus/total
  )

commute_la <- la_city_master_data %>%
  summarize(
    total = sum(B08303_001, na.rm = T),
    less_5 = sum(B08303_002, na.rm = T),
    commute_45_59 = sum(B08303_011, na.rm = T),
    commute_60_89 = sum(B08303_012, na.rm = T),
    commute_90_plus = sum(B08303_013),
    commute_5_14 = sum(across(all_of(commute_bins$commute_5_14)), na.rm = T),
    commute_15_24 = sum(across(all_of(commute_bins$commute_15_24)), na.rm = T),
    commute_25_34 = sum(across(all_of(commute_bins$commute_25_34)), na.rm = T),
    commute_35_44 = sum(across(all_of(commute_bins$commute_35_44)), na.rm = T)
    
  ) %>%
  mutate(
    share_less_5 = less_5/total,
    share_5_14 = commute_5_14/total,
    share_15_24 = commute_15_24/total,
    share_25_34 = commute_25_34/total,
    share_35_44 = commute_35_44/total,
    share_49_59 = commute_45_59/total,
    share_60_89 = commute_60_89/total,
    share_90_plus = commute_90_plus/total
  )


#reshape and combine df's
pico_long <- commute_pico %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "commute", values_to = "share")

city_long <- commute_la %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "commute", values_to = "share")

combined <- bind_rows(pico_long, city_long)


#custom labels
combined <- combined %>%
  mutate(
    commute = recode(commute,
                     "share_less_5"  = "Less than 5",
                     "share_5_14" = "5-14",
                     "share_15_24" = "15-24",
                     "share_25_34" = "25-34",
                     "share_35_44" = "35-44",
                     "share_45_59" = "45-59",
                     "share_60_89" = "60-89",
                     "share_90_plus" = "90 or more"
    ),
    commute = str_wrap(commute, width = 15),  # wrap first
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  ) %>%
  mutate(
    commute = factor(commute, levels = str_wrap(c(
      "Less than 5",
      "5-14",
      "15-24",
      "25-34",
      "35-44",
      "45-59",
      "60-89",
      "90 or more"
    ), width = 15))  # match the wrapped order
  )


# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))



#plot
ggplot(combined, aes(x = commute, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          
            color = "white",    
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )


############################################################## renter vs. owner occupied ##################################################

pico_tenure <- pico_master_data%>%
  summarize(
    total_occupied = sum(B25003_001, na.rm = T),
    owner_occupied = sum(B25003_002, na.rm = T),
    renter_occupied = sum(B25003_003, na.rm = T)
    
  )%>%
  mutate(
    share_owner = owner_occupied/total_occupied,
    share_renter = renter_occupied/total_occupied
  )

la_tenure <- la_city_master_data%>%
  summarize(
    total_occupied = sum(B25003_001, na.rm = T),
    owner_occupied = sum(B25003_002, na.rm = T),
    renter_occupied = sum(B25003_003, na.rm = T)
    
  )%>%
  mutate(
    share_owner = owner_occupied/total_occupied,
    share_renter = renter_occupied/total_occupied
  )

#reshape and combine df's
pico_long <- pico_tenure %>%
  select(starts_with("share_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "tenure", values_to = "share")

city_long <- la_tenure %>%
  select(starts_with("share_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("share_"), 
               names_to = "tenure", values_to = "share")

combined <- bind_rows(pico_long, city_long)


#custom labels
combined <- combined %>%
  mutate(
    tenure = recode(tenure,
                     "share_renter"  = "Renter occupied",
                     "share_owner" = "Owner occupied"
    ),
    tenure = str_wrap(tenure, width = 25),  # wrap first
    area = factor(area, levels = c("Pico", "LA City")),
    label = paste0(round(share * 100), "%")
  )

# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))



#plot
ggplot(combined, aes(x = tenure, y = share, fill = area)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = label),
            position = position_dodge(width = 0.8),
            vjust = 1.5,          
            color = "white",    
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = NULL,
    y = NULL,
    title = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "top",
    panel.grid.major.x = element_blank()
  )




#################################################################### HLA Voting #######################################################

hla_vote <- st_read(file.path(file_path, "Data/Measure HLA Vote/HLA.shp"))
library(lwgeom)

hla_vote <- st_make_valid(hla_vote)


CA_tracts <- st_read(file.path(file_path, "Hoover/Mapping/CA_census_Tracts.shp"), quiet = TRUE)
la_shp <- st_read(file.path(file_path, "Data/tl_2023_06_tract/tl_2023_06_tract.shp"), quiet = TRUE)
pico_buffer_tracts <- st_read(file.path(file_path, "Pico/Maps/pico_buffer_tracts.shp"), quiet = TRUE)

pico_tracts <- la_shp %>%
  filter(GEOID %in% pico_buffer_tracts$GEOID)


la_city_tracts <- st_read(file.path(file_path, "/Data/LA tracts/LA_City_2020_Census_Tracts_.shp"))%>%
  mutate(GEOID = paste0("06037", CT20))

#make sure crs matches
pico_tracts <- st_transform(pico_buffer_tracts, crs = 4326)
la_city_tracts <- st_transform(la_city_tracts, crs = 4326)
hla_vote <- st_transform(hla_vote, crs = 4326)



#find intersections with la and pico
voting_in_pico <- st_filter(hla_vote, pico_tracts)
voting_in_la <- st_filter(hla_vote, la_city_tracts)



# summing data
pico_hla <- voting_in_pico %>%
  summarize(
    total_yes = sum(V3_LOS_A_2, na.rm = T),
    total = sum(V3_LOS_A_4, na.rm = T)
  )%>%
  mutate(
    vote_share = total_yes/total
  )

la_hla  <- voting_in_la %>%
  summarize(
    total_yes = sum(V3_LOS_A_2, na.rm = T),
    total = sum(V3_LOS_A_4, na.rm = T)
  )%>%
  mutate(
    vote_share = total_yes/total
  )


#reshape and combine df's
pico_long <- pico_hla %>%
  select(starts_with("vote_")) %>%
  mutate(area = "Pico") %>%
  pivot_longer(cols = starts_with("vote_"), 
               names_to = "vote", values_to = "share")

city_long <- la_hla %>%
  select(starts_with("vote_")) %>%
  mutate(area = "LA City") %>%
  pivot_longer(cols = starts_with("vote_"), 
               names_to = "vote", values_to = "share")

combined <- bind_rows(pico_long, city_long)%>%
  mutate(
    label = paste0(round(share * 100, 1), "%")  # Add percentage labels
  )



# custom colors per LA DOT style guide
colors <- c("Pico" = rgb(135, 160, 180, maxColorValue = 255), 
            "LA City" = rgb(26, 67, 120, maxColorValue = 255))


#plot
ggplot(combined, aes(x = area, y = share, fill = area)) +
  geom_col(width = 0.6) +
  geom_text(aes(label = label),
            vjust = 1.5,
            color = "white",
            size = 4) +
  scale_fill_manual(values = colors, name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     limits = c(0, 1)) +
  labs(x = NULL, y = NULL, title = NULL) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "none",
    panel.grid.major.x = element_blank()
  )


