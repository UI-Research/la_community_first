...
##hoover tracts##
hoover_buffer_tracts <- st_read(file.path(file_path, "Hoover/Mapping/hoover_buffer_census_tracts.shp"))
st_crs(hoover_buffer_tracts)
st_crs(hoover_buffer_tracts) <- 4326


hoover_tracts <- la_shp %>%
  filter(GEOID %in% hoover_buffer_tracts$GEOID)

hoover_tracts %>% pull(GEOID)

#streets
#create specific set of streets we want to see
bbox <- st_bbox(hoover_tracts)
#load in
...
major_streets <- streets_lines %>%
  ...
  st_transform(st_crs(hoover_tracts))

#create a version that clips the streets to the focus area
major_streets_clipped <- st_intersection(major_streets, st_union(st_geometry(hoover_tracts)))
...
pop_wide_hoover <- sex_by_age %>%
  select(GEOID, variable, estimate, geometry) %>%
  filter(variable == "B01001_001", GEOID %in% hoover_tracts$GEOID) %>%
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/population_histogram_hoover.png"), width = 8, height = 6, dpi = 300)

#create map of population
population_map_hoover <- ggplot(pop_wide_hoover) +
...
ggsave(file.path(file_path, "Hoover/Outputs/population_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
age_wide_hoover <- sex_by_age %>%
  filter(variable %in% unlist(age_bins), GEOID %in% hoover_tracts$GEOID)%>%
  group_by(GEOID)%>%
...
age_totals_hoover <- age_wide_hoover %>%
  st_drop_geometry() %>%
...
pop_by_age_group_hoover <- ggplot(age_totals_hoover, aes(x = age_group, y = population, fill = age_group)) +
  geom_col() +
  labs(
    x = "Age Group",
    y = "Population"
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/age_chart_hoover.png"), width = 14, height = 6, dpi = 300)
...
ethnicity_wide_hoover <- ethnicity %>%
...
#reformat for ggplot
ethnicity_long_hoover <- ethnicity_wide_hoover %>%
  filter(GEOID == "Total") %>%
...
#create a barchart
ethnicity_chart_hoover <- ggplot(ethnicity_long_hoover, aes(x = reorder(group, -percent), y = percent, fill = group)) +
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/ethnicity_chart_hoover.png"), width = 14, height = 6, dpi = 300)
...
#filter for hoover tracts 
country_origin_filtered_hoover <- country_origin %>%
  filter(GEOID %in% hoover_tracts$GEOID)

country_of_origin_totals_hoover <- country_origin_filtered_hoover%>%
  group_by(variable) %>%
...
#after reviewing the table above, edit to manually pull out the highest (in this case I chose the top countries with more than 500). This is done manually because some of the variables include 'Total', 'Asia', 'Central America', etc.
country_origin_top_hoover <- country_origin_filtered_hoover %>%
  filter(
    GEOID %in% hoover_tracts$GEOID
  ) %>%
  mutate(country = case_when(
    variable == "B05006_158" ~ "Guatemala",
    variable == "B05006_157" ~ "El Salvador",
    variable == "B05006_160" ~ "Mexico",
    variable == "B05006_155" ~ "Belize",
    variable == "B05006_161" ~ "Nicaragua",
    variable == "B05006_159" ~ "Honduras",
  )) %>%
  group_by(country) %>%
  summarise(total = sum(estimate, na.rm = TRUE), .groups = "drop")

write.csv(country_origin_top_hoover, file = file.path(file_path, "Hoover/Outputs/country_origin_top_hoover.csv"), row.names = FALSE)
...
#binding with hoover tracts to limit our data to the study area
hoover_disability <- disability %>%
  filter(GEOID %in% hoover_tracts$GEOID)

#add geometry from census tracts to create a map
st_geometry(hoover_disability) <- st_geometry(hoover_tracts[match(hoover_disability$GEOID, hoover_tracts$GEOID), ])


#creating custom bins for a map of %pop w/ disability and make each of the bins a factor
map_disability_hoover <- hoover_disability %>% 
  mutate(bin_disability = factor(
    cut(
      total_disability,
...
#making the map
plot <-ggplot()+
  geom_sf(map_disability_hoover, mapping = aes(fill = bin_disability), show.legend = TRUE) +
  geom_sf(data = hoover_buffer_tracts, color = "yellow", alpha = 0, lwd = 1) + # Adding the buffer zone as a transparent overlay
...
print(plot) #view map
ggsave(file.path(file_path, "Hoover/Outputs/disability_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
income_wide_hoover<- median_income %>%
  filter(variable == "B19013_001", GEOID %in% hoover_tracts$GEOID) %>%
  mutate(income = estimate) %>%
  select(GEOID, income, NAME, moe, geometry)

#create a map based on incomes
income_map_hoover <- ggplot(income_wide_hoover) +
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/income_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
poverty_wide_hoover <- poverty_level %>%
...
sum(poverty_wide_hoover$below_pov)

total_share_below_pov <- sum(poverty_wide_hoover$below_pov, na.rm = TRUE) / sum(poverty_wide_hoover$total_pop, na.rm = TRUE)

#Map of tracts by poverty level status
poverty_map_hoover <- ggplot(poverty_wide_hoover) +
...
ggsave(file.path(file_path, "Hoover/Outputs/poverty_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
employment_wide_hoover <- employment_status %>%
  filter(variable %in% employment_bins$variable, GEOID %in% hoover_tracts$GEOID) %>%
...
employment_bar_data <- employment_wide_hoover %>%
...
employment_bar_chart_hoover <- ggplot(employment_bar_data, aes(x = employment_status, y = share, fill = employment_status)) +
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/employment_bar_chart_hoover.png"), width = 14, height = 6, dpi = 300)
...
#create figure
gender_bar_chart_hoover <- ggplot(
  tibble(
    status = c("Male", "Female"),
    share = c(
      gender_wide_hoover$share_male[gender_wide_hoover$GEOID == "Total"],
      gender_wide_hoover$share_female[gender_wide_hoover$GEOID == "Total"]
    )
  ),
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/gender_bar_chart_hoover.png"), width = 14, height = 6, dpi = 300)
...
family_wide_hoover <- family_structure %>%
  filter(variable %in% unlist(family_bins), GEOID %in% hoover_tracts$GEOID) %>%
  mutate(family_structure = case_when(
    ...
    variable %in% family_bins$single_children ~ "Not married with children under 18",
    variable %in% family_bins$single_no_children ~ "Not married without children under 18",
    TRUE ~ NA_character_ 
  )) %>%
...
family_totals <- family_wide_hoover %>%
  summarise(across(starts_with("Married"), ~sum(.x, na.rm = TRUE)), 
            across(starts_with("Not"), ~sum(.x, na.rm = TRUE))) %>%
...
#create a bar chart
pop_by_family_type_hoover <- ggplot(family_totals, aes(x = family_structure, y = population, fill = family_structure)) +
  geom_col() +
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/pop_by_family_type_hoover.png"), width = 14, height = 6, dpi = 300)
...
hearing_diff <- 
  ...
  filter(GEOID %in% hoover_tracts$GEOID) #limiting to hoover tracts

#add geometry for map
st_geometry(hearing_diff) <- st_geometry(hoover_tracts[match(hearing_diff$GEOID, hoover_tracts$GEOID), ])
class(hearing_diff)
...
print(plot)
ggsave(file.path(file_path, "Hoover/Outputs/hearing_diff_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
#add geometry for map
st_geometry(vision_diff) <- st_geometry(hoover_tracts[match(vision_diff$GEOID, hoover_tracts$GEOID), ])
class(vision_diff)
...
ggsave(file.path(file_path, "Hoover/Outputs/vision_diff_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
#add geometry for map
st_geometry(ambulatory_diff) <- st_geometry(hoover_tracts[match(ambulatory_diff$GEOID, hoover_tracts$GEOID), ]
)
...
# save
ggsave(file.path(file_path, "Hoover/Outputs/ambulatory_diff_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
st_geometry(internet_subs) <- st_geometry(hoover_tracts[match(internet_subs$GEOID, hoover_tracts$GEOID), ])
class(internet_subs)
...
ggsave(file.path(file_path, "Hoover/Outputs/internet_access_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
#add geometry for map
st_geometry(tech_access) <- st_geometry(hoover_tracts[match(tech_access$GEOID, hoover_tracts$GEOID), ])
class(tech_access)
...
ggsave(file.path(file_path, "Hoover/Outputs/tech_access_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
#saving
ggsave(file.path(file_path, "Hoover/Outputs/tech_access_bar_hoover.png"),  width = 9, height = 3)     
...
#adding geometry for map
st_geometry(tenure) <- st_geometry(hoover_tracts[match(tenure$GEOID, hoover_tracts$GEOID), ])
class(tenure)

#creating custom bins for a map of share of occupied HH that are renter
map_tenure <- tenure%>% 
...
ggsave(file.path(file_path, "Hoover/Outputs/share_renter_occupied_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
# Pivot  data wider
language_wide_hoover <- language %>%
  filter(variable %in% unlist(language_bins), GEOID %in% hoover_tracts$GEOID) %>%
...
language_totals <- language_wide_hoover %>%
  select(-GEOID, -NAME) %>%
...
#create a bar chart
pop_by_language_skill_hoover <- ggplot(language_totals, aes(x = language_skill, y = population, fill = language_skill)) +
  geom_col() +
  geom_text(aes(label = scales::comma(population)), vjust = -0.5, size = 5) +
  labs(
    x = NULL,
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/pop_by_language_skill_hoover.png"), width = 14, height = 6, dpi = 300)

#repeat the above but calculating percentages
# Calculate percentages for each language skill category
language_totals_pct <- language_wide_hoover %>%
  select(-GEOID, -NAME) %>%
  summarise(across(everything(), ~sum(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = everything(),
...
pop_by_language_skill_pct_hoover <- ggplot(language_totals_pct, aes(x = language_skill, y = percent, fill = language_skill)) +
  geom_col() +
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/pop_by_language_skill_pct_hoover.png"), width = 14, height = 6, dpi = 300)
...
#calculate totals for bar chart
language_spoken_totals <- spoken_language_wide_hoover %>%
  select(-GEOID, -NAME) %>%
  summarise(across(everything(), ~sum(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = everything(),
...
# Plot as a percentage bar chart
pop_by_language_spoken_pct_hoover <- ggplot(language_spoken_totals_pct, aes(x = language_spoken, y = percent, fill = language_spoken)) +
  geom_col() +
...
#save
ggsave(file.path(file_path, "Hoover/Outputs/pop_by_language_spoken_pct_hoover.png"), width = 14, height = 6, dpi = 300)

#create a map
spoken_language_wide_hoover_sf <- spoken_language_wide_hoover %>%
  mutate(
    share_english_only = `English Only` / Total,
    share_spanish = Spanish / Total,
  ...
  left_join(select(hoover_tracts, GEOID, geometry), by = "GEOID") %>%
  st_as_sf()

spanish_map_hoover <- ggplot(spoken_language_wide_hoover_sf) +
...
#adding geometry for map
st_geometry(renter_burden) <- st_geometry(hoover_tracts[match(renter_burden$GEOID, hoover_tracts$GEOID), ])
class(renter_burden)
...
plot <-ggplot()+
  ...
  geom_sf(data = hoover_buffer_tracts, color = "black", alpha = 0, lwd = 1) + # Adding the buffer zone as a transparent overlay
  geom_sf(data = major_streets_clipped, color = "gray20", size = 0.5) + #adding streets to map
  scale_fill_manual(
...
ggsave(file.path(file_path, "Hoover/Outputs/med_share_inc_housing_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
ggsave(file.path(file_path, "Hoover/Outputs/rent_burdened_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
ggsave(file.path(file_path, "Hoover/Outputs/severely_rent_burdened_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
# adding geometry for map
st_geometry(owner_burden) <- st_geometry(hoover_tracts[match(owner_burden$GEOID, hoover_tracts$GEOID), ])
class(owner_burden)
...
plot <- ggplot() +
  ...
  geom_sf(data = hoover_buffer_tracts, color = "black", alpha = 0, lwd = 1) + # transparent buffer zone
...
ggsave(file.path(file_path, "Hoover/Outputs/owner_med_share_inc_housing_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
plot <- ggplot() +
  ...
  geom_sf(data = hoover_buffer_tracts, color = "yellow", alpha = 0, lwd = 1) +
...
ggsave(file.path(file_path, "Hoover/Outputs/owner_share_inc_housing_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
ggsave(file.path(file_path, "Hoover/Outputs/owner_share_severe_rent_burden_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
### 40 of 46 hoover tracts are disadvantaged
...
# adding geometry for map
st_geometry(displacement_risk) <- st_geometry(hoover_tracts[match(displacement_risk$GEOID, hoover_tracts$GEOID), ])
...
#saving
ggsave(file.path(file_path, "hoover/Outputs/displacement_risk_bar_hoover.png"),  width = 8, height = 2.5) 
...
#adding geometry for map
st_geometry(vehicles) <- st_geometry(hoover_tracts[match(vehicles$GEOID, hoover_tracts$GEOID), ])
class(vehicles)
...
ggsave(file.path(file_path, "Hoover/Outputs/share_no_car_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
ggsave(file.path(file_path, "Hoover/Outputs/transportation_means_hoover.png"), width = 14, height = 6, dpi = 300)
...
#saving
ggsave(file.path(file_path, "Hoover/Outputs/means_of_transit_bar_hoover.png"),  width = 8, height = 3)  
...
# Sum columns 1 to 9 from the original dataframe
#create a df of totals across all tracts and compute shares of each commute bin
travel_time_sums <- colSums(travel_time[, 2:10], na.rm = TRUE)
travel_time_sums <- data.frame(t(travel_time_sums))

# Generating shares for transport types, skipping the first column (total)
for (i in 2:9) {  # Start from column 2 to avoid dividing by the total column
...
travel_time_sums_long <- travel_time_sums %>%
  ...
  pivot_longer(cols = everything(), names_to = "Variable", values_to = "Value") %>%
  mutate(
    Variable = recode(Variable, !!!custom_labels)  # Recode the 'Variable' column with custom labels
  ) %>%
  mutate(
    Variable = factor(Variable, levels = custom_labels)  # Set the order of levels
  )

#bar chart
plot <- ggplot(travel_time_sums_long) +
  geom_col(mapping = aes(x = Variable, y = Value), position = "dodge") +
  geom_text(mapping = aes(x = Variable, y = Value, label = scales::percent(Value, accuracy = 1)),
            vjust = -0.5, size = 3.5, family = "Lato") +
  scale_y_continuous(
  ...
  labs(
    ...
    y = "Share of residents in hoover census tracts"
  ) +
  theme(
    legend.position = "top",
    legend.text = element_text(size = 12, family = "Lato"),
...
#saving
ggsave(file.path(file_path, "Hoover/Outputs/commute_time_bar_hoover.png"),  width = 8, height = 3)  
...
#add geometry for map
st_geometry(travel_time) <- st_geometry(hoover_tracts[match(travel_time$GEOID, hoover_tracts$GEOID), ])
class(travel_time)
...
ggsave(file.path(file_path, "Hoover/Outputs/commute_map_hoover.png"), width = 14, height = 6, dpi = 300)

#load in data from TIMS
crashes_hoover <- read.csv(file.path(file_path, "Hoover/Outputs/crash_data_hoover_blvd.csv"))
##Collisions - not sure about best data here
#convert to an SF
crashes_hoover_sf <- st_as_sf(
  crashes_hoover,
...
crash_map_hoover <- ggplot() +
  geom_sf(data = hoover_buffer_tracts, fill = "lightgray", color = "gray40", size = 0.3) +
  geom_sf(data = major_streets_clipped, color = "orange", size = 0.6) +
  geom_sf(data = crashes_hoover_sf, color = "red", size = 1, alpha = 0.7) +
  # labs(title = "Crash Locations in hoover Area",
...
print(crash_map_hoover) #view map

#save
ggsave(file.path(file_path, "Hoover/Outputs/crash_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
#add geometry for map
st_geometry(ht_index) <- st_geometry(hoover_tracts[match(ht_index$GEOID, hoover_tracts$GEOID), ])
class(ht_index)
...
ggsave(file.path(file_path, "Hoover/Outputs/ht_ami_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
ggsave(file.path(file_path, "Hoover/Outputs/ht_80ami_map_hoover.png"), width = 14, height = 6, dpi = 300)
...
hoover_buffer_tracts_1 <- st_transform(hoover_buffer_tracts, crs = 4326)
hla_vote <- st_transform(hla_vote, crs = 4326)

#Make geometries valid
hoover_buffer_tracts_1 <- st_make_valid(hoover_buffer_tracts_1)
hla_vote <- st_make_valid(hla_vote)


#find intersection
voting_intersection_hoover <- st_intersection(hoover_buffer_tracts_1, hla_vote)

library(lwgeom)

voting_map_hoover <- ggplot() +
  geom_sf(data = hoover_buffer_tracts_1) +
  geom_sf(data = voting_intersection_hoover, aes(fill = V3_LOS_A_5), color = NA) +
  geom_sf(data = major_streets_clipped, color = "gray20", size = 0.5) + #adding streets to map
  scale_fill_gradientn(
...
ggsave(file.path(file_path, "Hoover/Outputs/voting_map_hoover.png"), width = 14, height = 6, dpi = 300)
